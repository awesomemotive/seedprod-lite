  ...
  <div>
    ...
    <div v-if="view == 1">
      ...
      <div class="seedprod-sidebar-action-body" v-if="section_1">
        ...
        <div class="sp-form-group">
          <label>{{txt_9}}</label>
          <select v-model="block.settings.embedType" class="sp-form-select">
            <option value="post">{{txt_10}}</option>
            <option value="video">{{txt_11}}</option>
            <option value="comment">{{txt_12}}</option>
          </select>
        </div>

        <div class="sp-form-group" v-if="block.settings.embedType == 'post'">
              <div><label>{{txt__8}}</label></div>
              <input
                :name="'Link'"
                type="text"
                v-model="block.settings.postlinkurl"
                v-validate="'required'"
        ...
        <div class="sp-form-group" v-if="block.settings.embedType == 'video'">
              <div><label>{{txt__9}}</label></div>
              <input
                :name="'Link'"
                type="text"
                v-model="block.settings.videolinkurl"
                v-validate="'required'"
                class="sp-form-input sp-w-full"
              />
              <span
        ...
        <div class="sp-form-group" v-if="block.settings.embedType == 'comment'">
              <div><label>{{txt__10}}</label></div>
              <input
                :name="'Link'"
                type="text"
                v-model="block.settings.commentlinkurl"
                v-validate="'required'"
                class="sp-form-input sp-w-full"
              />
              <span
        ...
        <div
          ...
          v-if="block.settings.embedType == 'post' || block.settings.embedType == 'video'"
        >
          <label>{{ txt__1 }}</label>
          <div
          ...
          >
            <toggle-button
              ...
              v-model="block.settings.fullPost"
            />&nbsp;
            <span
              v-if="block.settings.fullPost"
        ...
        <div v-if="block.settings.embedType == 'video'">
            ...
            >
              ...
              >
                <toggle-button
                  ...
                  :sync="true"
                  style="margin-bottom: 0"
                  v-model="block.settings.fullScreen"
                />&nbsp;
                <span
                  v-if="block.settings.fullScreen"
                  class="sp-text-11px sp-uppercase sp-pl-1 sp-font-bold"
                  >{{ txt__2 }}</span
                >
                <span
            ...
            <div class="sp-form-group sp-flex sp-justify-between sp-items-center sp-mt-4">
              ...
              >
                <toggle-button
                  ...
                  :sync="true"
                  style="margin-bottom: 0"
                  v-model="block.settings.captions"
                />&nbsp;
                <span
                  v-if="block.settings.captions"
                  class="sp-text-11px sp-uppercase sp-pl-1 sp-font-bold"
                  >{{ txt__2 }}</span
                >
                <span
        ...
        <div
          class="sp-form-group sp-flex sp-justify-between sp-items-center sp-mt-4"
           v-if="block.settings.embedType == 'comment'"
        >
          <label>{{ txt__7 }}</label>
          <div
            class="sp-bg-neutral-5 sp-flex sp-items-center sp-justify-center"
          >
            <toggle-button
              ...
              :sync="true"
              style="margin-bottom: 0"
              v-model="block.settings.parentComment"
            />&nbsp;
            <span
              v-if="block.settings.parentComment"
              class="sp-text-11px sp-uppercase sp-pl-1 sp-font-bold"
              >{{ txt__2 }}</span
            >
            <span
...
import FacebookSettings from '@/components/FacebookSettings.vue';

export default {
  name: 'FacebookEmbedOptions',
  mixins: [ helpers ],
  inject: {
    $validator: '$validator',
  },
  data() {
    return {
      txt__1: __( 'Include full post', process.env.VUE_APP_TEXTDOMAIN ),
      txt__2: __( 'Yes', process.env.VUE_APP_TEXTDOMAIN ),
      txt__3: __( 'No', process.env.VUE_APP_TEXTDOMAIN ),
      txt__4: __( 'Allow Full Screen', process.env.VUE_APP_TEXTDOMAIN ),
      txt__5: __( 'Autoplay', process.env.VUE_APP_TEXTDOMAIN ),
      txt__6: __( 'Captions', process.env.VUE_APP_TEXTDOMAIN ),
      txt__7: __( 'Parent Comment', process.env.VUE_APP_TEXTDOMAIN ),
      txt__8: __( 'Post Link', process.env.VUE_APP_TEXTDOMAIN ),
      txt__9: __( 'Video Link', process.env.VUE_APP_TEXTDOMAIN ),
      txt__10: __( 'Comment Link', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt_3: __( 'Facebook Setting', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt_9: __( 'Type', process.env.VUE_APP_TEXTDOMAIN ),
      txt_10: __( 'Post', process.env.VUE_APP_TEXTDOMAIN ),
      txt_11: __( 'Video', process.env.VUE_APP_TEXTDOMAIN ),
      txt_12: __( 'Comment', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt_21: __( 'Height', process.env.VUE_APP_TEXTDOMAIN ),

      //txt_20: __("Font Size", process.env.VUE_APP_TEXTDOMAIN),
      //txt_21: __("Text Shadow", process.env.VUE_APP_TEXTDOMAIN),
      txt_22: __( 'None', process.env.VUE_APP_TEXTDOMAIN ),
  ...
  mounted() {
    ...
    setTimeout( function() {
      self.onFBSettingChange();
    }, 500 );
  },
  created() {},
  methods: {
    onFBSettingChange() {
      FB.XFBML.parse( document.getElementById( '#sp-fbembed-' + this.block.id ) );
    },
  },
  watch: {
    'block.settings.embedType': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.fullPost': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.fullScreen': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.autoPlay': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.captions': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.parentComment': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.postlinkurl': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.videolinkurl': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.commentlinkurl': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
  },
  components: {
