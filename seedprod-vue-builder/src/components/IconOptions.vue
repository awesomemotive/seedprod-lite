    ...
    <div v-if="view == 1">
      ...
      <div class="seedprod-sidebar-action-body" v-if="section_1">
        ...
        <div class="sp-form-group sp-mt-4">
          <ColorPicker :label="txt_15" :color="block.settings.color" v-model="block.settings.color" />
        </div>

        <div class="sp-form-group">
          <label>{{txt_6}}</label>
          <SliderControl v-model="block.settings.fontSize" :min="10" :max="150" :step="1" />
        </div>
      </div>
    </div>
      ...
      <div class="seedprod-sidebar-action-body" v-if="section_4">
        <div class="sp-form-group">
          ...
          <select v-model="block.settings.textShadow" class="sp-form-select">
            <option value>{{txt_8}}</option>
            <option value="1">{{txt_9}}</option>
            <option value="2">{{txt_10}}</option>
            <option value="3">{{txt_11}}</option>
            <option value="4">{{txt_12}}</option>
            <option value="5">{{txt_13}}</option>
            <option value="6">{{txt_14}}</option>
          </select>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      txt_3: __( 'Styles', process.env.VUE_APP_TEXTDOMAIN ),
      txt_4: __( 'Icon', process.env.VUE_APP_TEXTDOMAIN ),
      txt_5: __( 'Align', process.env.VUE_APP_TEXTDOMAIN ),
      txt_6: __( 'Size', process.env.VUE_APP_TEXTDOMAIN ),
      txt_7: __( 'Shadow', process.env.VUE_APP_TEXTDOMAIN ),
      txt_8: __( 'None', process.env.VUE_APP_TEXTDOMAIN ),
      txt_9: __( 'Hairline', process.env.VUE_APP_TEXTDOMAIN ),
      txt_10: __( 'Small', process.env.VUE_APP_TEXTDOMAIN ),
      txt_11: __( 'Medium', process.env.VUE_APP_TEXTDOMAIN ),
      txt_12: __( 'Large', process.env.VUE_APP_TEXTDOMAIN ),
      txt_13: __( 'X Large', process.env.VUE_APP_TEXTDOMAIN ),
      txt_14: __( '2X Large', process.env.VUE_APP_TEXTDOMAIN ),
      txt_15: __( 'Color', process.env.VUE_APP_TEXTDOMAIN ),
      view: 1,
      section_1: true,
      section_2: false,
      section_3: false,
  ...
  mounted() {
    let self = this;
    lodash.forOwn( this.shared.block_templates.icon, function( value, key ) {
