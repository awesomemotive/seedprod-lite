<template>
  <div :id="'sp-'+block.id" class="sp-form-wrapper" :style="formWrapperStyleObject" :class="{'sp-has-form':block.settings.formId !== ''}" :data-mobile-css="mobileFormStylesCal">
    ...
    <div v-if="shared.plugins_installed['wpforms-pro'].status === 0">
      <div
        v-if="plugins['wpforms'].status_code !== 1"
      ...
      >
        ...
        <br />
        <a
          v-if="plugins['wpforms'].status_code === 0 && doing_install === false"
          ...
          :href="seedprod_form_install_link"
          target="_blank"
          @click.prevent="plugin_action('wpforms')"
        >
          <span class="sp-text-white">{{txt_3}} WPForms</span>
        </a>
        ...
        <a
          v-if="plugins['wpforms'].status_code === 2 && doing_install === false"
          class="sp-mt-2 sp-bg-primary hover:sp-bg-primary-lighter sp-text-white sp-font-semibold sp-py-3 sp-px-5 sp-rounded sp-leading-none sp-inline-block sp-font-sans"
          :href="seedprod_form_activate_link"
          target="_blank"
          @click.prevent="plugin_action('wpforms')"
        >
          <span class="sp-text-white">{{txt_4}} WPForms</span>
          <span v-if="doing_install === true">
            Installing
            <i class="fas fa-spinner fa-spin"></i>
          </span>
    ...
    <div
      v-if="plugins['wpforms'].status_code === 1 || shared.plugins_installed['wpforms-pro'].status === 1"
    >
      <div class="sp-preview-hidden">[wpforms id="{{block.settings.formId}}"]</div>
      <span class="sp-hidden">START-REMOVE</span>
      <div v-html="preview" :style="formPreviewStyleObject"></div>
      <div v-show="!preview" class="sp-text-center sp-text-sm">
        ...
        <div v-if="forms.length">
          <select v-model="block.settings.formId" v-on:change="selectFormPreviewOnChange" class="sp-form-select sp-w-full" style="max-width: 300px;">
            ...
            <option v-for="form in forms" v-bind:value="form.ID" :key="form.ID">{{ form.post_title }}</option>
          </select>
        </div>
...
export default {
  ...
  data() {
    return {
      txt_1: __( 'Install Contact Form plugin:', process.env.VUE_APP_TEXTDOMAIN ),
      txt_3: __( 'Install', process.env.VUE_APP_TEXTDOMAIN ),
      txt_4: __( 'Activate', process.env.VUE_APP_TEXTDOMAIN ),
      txt_5: __( 'Contact Form', process.env.VUE_APP_TEXTDOMAIN ),
      txt_6: __(
        '(This shortcode will be rendered on the live preview.)',
        process.env.VUE_APP_TEXTDOMAIN
      ),
      txt_7: __( 'Select a Form', process.env.VUE_APP_TEXTDOMAIN ),
      txt_8: __( 'You can use WPForms to build contact forms, surveys, payment forms, and more with just a few clicks.', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      placeholder: seedprod_data.wpforms ? seedprod_data.wpforms.placeholder : '',
      preview: '',
      forms: [],
    };
  },
  methods: {
        ...
        .then( function( response ) {
          ...
          if ( 'activate' == type ) {
            if ( response.data.success ) {
              ...
              self.placeholder = '<img src=\'' + self.shared.plugin_path + 'public/img/plugin-wpforms.png\' width=\'80px\' alt=\'\'/>';
              self.$forceUpdate();
              EventBus.$emit( 'pluginActivated' );
            } else {
              self.$swal({
                imageUrl:
                  self.shared.plugin_path + 'public/svg/error-24px-white.svg',
    ...
    loadForm( id, args ) {
      ...
      self.axios
        .get( 'admin-ajax.php?action=' + self.shared.page_path + '_get_wpform&_wpnonce=' + seedprod_nonce + '&form_id=' + id + '&form_title=' + form_title + '&form_description=' + form_description )
        .then( function( response ) {
          if ( response.data ) {
  ...
  mounted: function() {
    ...
    lodash.forOwn( this.shared.block_templates.contactform, function(
      value,
      key
    ) {
    ...
    EventBus
      .$on( 'formsLoaded', function( forms ) {
        self.forms = forms.length ? forms : [];
      })
      .$on( 'formSelected', function( formSettings ) {
        self.loadForm( formSettings.form_id, formSettings );
      })
      .$on( 'iframeClosed', function( formId, obs ) {
        obs.disconnect();
      })
      .$on( 'createNewLinkClicked', function( url ) {
        self.create_new_form = true;
        self.initObserver( url );
      })
      .$on( 'editLinkClicked', function( url ) {
        self.initObserver( url );
      });
  ...
  computed: {
    ...
    formWrapperStyleObject: function() {
      let mt = '';
    ...
    formPreviewStyleObject: function() {
