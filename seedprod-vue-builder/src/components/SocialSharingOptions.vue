  ...
  <div>
    ...
    <div v-if="view == 1">
      ...
      <div class="seedprod-sidebar-action-body" v-if="section_1">
          ...
          <draggable :list="block.settings.items"  handle=".sp-drag-drop">
            ...
            >
              <div class="sp-flex sp-items-center">
                <select v-model="item.type" class="sp-form-select" style="width:160px">
                  <option value>{{txt_6}}</option>
                  <option value="facebook">{{txt_7}}</option>
                  <option value="twitter">{{txt_8}}</option>
                  <option value="linkedin">{{txt_9}}</option>
                  <option value="pinterest">{{txt_10}}</option>
                </select>

                <span v-show="current_index === index" class="sp-drag-drop sp-text-neutral-40 hover:sp-text-neutral-80 sp-ml-3 sp-cursor-pointer">
                  <i class="fas fa-arrows-alt"></i>
                </span>

                <button
                  v-tooltip="{content:'Settings',delay:{ show: 1500, hide: 0 }}"
              ...
              >
                ...
                <div v-if="item.type == 'twitter'" class="sp-form-group">
                  <div>
                    <label>{{txt_12}}</label>
                  </div>
                  <input type="text" v-model="item.meta" class="sp-form-input" />
                </div>
                <div v-if="item.type == 'pinterest'" class="sp-form-group">
                  <div>
                    <label>{{txt_13}}</label>
                  </div>
                  <ImageControl v-model="item.meta" :block="block" prefix="img" />
                </div>
              </div>
            </div>
          </draggable>
        ...
        <div class="sp-form-group">
          <label>{{txt_14}}</label>
          <select v-model="block.settings.size" class="sp-form-select">
            <option value="sm">{{txt_17}}</option>
            <option value="md">{{txt_18}}</option>
            <option value="lg">{{txt_19}}</option>
          </select>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      txt_3: __( 'Social Sharing', process.env.VUE_APP_TEXTDOMAIN ),
      txt_4: __( 'Add New Share', process.env.VUE_APP_TEXTDOMAIN ),
      txt_5: __( 'Styles', process.env.VUE_APP_TEXTDOMAIN ),
      txt_6: __( 'Select a Type', process.env.VUE_APP_TEXTDOMAIN ),
      txt_7: __( 'Facebook', process.env.VUE_APP_TEXTDOMAIN ),
      txt_8: __( 'Twitter', process.env.VUE_APP_TEXTDOMAIN ),
      txt_9: __( 'LinkedIn', process.env.VUE_APP_TEXTDOMAIN ),
      txt_10: __( 'Pinterest', process.env.VUE_APP_TEXTDOMAIN ),
      txt_11: __( 'Label', process.env.VUE_APP_TEXTDOMAIN ),
      txt_12: __( 'Tweet Text', process.env.VUE_APP_TEXTDOMAIN ),
      txt_13: __( 'Image', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt_22: __( 'Align', process.env.VUE_APP_TEXTDOMAIN ),
      txt_23: __( 'Space Between', process.env.VUE_APP_TEXTDOMAIN ),
      txt_24: __( 'Shadow', process.env.VUE_APP_TEXTDOMAIN ),
      current_index: false,
      view: 1,
      section_1: true,
      section_2: false,
  ...
  mounted() {
    let self = this;
    lodash.forOwn( this.shared.block_templates.socialsharing, function(
      value,
      key
    ) {
