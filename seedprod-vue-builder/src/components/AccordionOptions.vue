  ...
  <div>
    ...
    toolbar1:
      ...
      <div class="seedprod-sidebar-action-body" v-if="section_2">
        ...
        <div class="sp-form-group">
          <label>{{txt_7}}</label>
          <AlignControl v-model="block.settings.align" icons="alt" :hidecenter="true"/>
        </div>
        ...
        <div class="sp-form-group">
          <label>{{txt_19}}</label>
          <IconPicker v-model="block.settings.closedIcon" />
        </div>

        <div class="sp-form-group">
          <ColorPicker
            :label="txt_20"
            :color="block.settings.openIconColor"
            v-model="block.settings.openIconColor"
          />
        </div>
        <div class="sp-form-group">
          <label>{{txt_21}}</label>
          <IconPicker v-model="block.settings.openIcon" />
        </div>
    ...
    <div v-if="view == 2" class="sp-flex sp-items-center sp-flex-col sp-pb-8 sp-optin-templates">
      ...
      <!-- Rounded Official -->
      <button class="sp-mt-4" @click="apply_template(0)">
        <i class="fa-fw fas fa-check" :style="{ color: block.settings.iconColor }"></i>
      </button>

      <button class="sp-mt-4 sp-bulletlist-block-template-1" @click="apply_template(1)">
        <span class="sp-icon-wrapper">
          <i class="fa-fw fas fa-check" :style="{ color: block.settings.iconColor }"></i>
        </span>
      </button>

      <button class="sp-mt-4 sp-bulletlist-block-template-2" @click="apply_template(2)">
        <span class="sp-icon-wrapper">
          <i class="fa-fw fas fa-check" :style="{ color: block.settings.iconColor }"></i>
        </span>
      </button>

      <button class="sp-mt-4 sp-bulletlist-block-template-3" @click="apply_template(3)">
        <span class="sp-icon-wrapper">
          <i class="fa-fw fas fa-check" :style="{ color: block.settings.iconColor }"></i>
        </span>
      </button>

      <button class="sp-mt-4 sp-bulletlist-block-template-4" @click="apply_template(4)">
        <span class="sp-icon-wrapper">
          <i class="fa-fw fas fa-check" :style="{ color: block.settings.iconColor }"></i>
        </span>
      </button>

      <button class="sp-mt-4 sp-bulletlist-block-template-5" @click="apply_template(5)">
        <span class="sp-icon-wrapper">
          <i class="fa-fw fas fa-check" :style="{ color: block.settings.iconColor }"></i>
        </span>
      </button>

      <button class="sp-mt-4 sp-bulletlist-block-template-6" @click="apply_template(6)">
        <span class="sp-icon-wrapper">
          <i class="fa-fw fas fa-check" :style="{ color: block.settings.iconColor }"></i>
        </span>
      </button>
    ...
    <div v-if="view == 3">
      ...
      <div class="seedprod-sidebar-action-body" v-if="section_4">
        ...
        <div class="sp-form-group">
          <ColorPicker
            :label="txt_23"
            :color="block.settings.headerOpenColor"
            v-model="block.settings.headerOpenColor"
          />
        </div>
        ...
        <div class="sp-form-group">
          <ColorPicker
            :label="txt_24"
            :color="block.settings.dividerColor"
            v-model="block.settings.dividerColor"
          />
        </div>
        ...
        <div class="sp-form-group">
          ...
          <select v-model="block.settings.shadow" class="sp-form-select">
            <option value>{{txt_114}}</option>
            <option value="1">{{txt_115}}</option>
            <option value="2">{{txt_116}}</option>
            <option value="3">{{txt_117}}</option>
            <option value="4">{{txt_118}}</option>
            <option value="5">{{txt_119}</option>
            <option value="6">{{txt_120}}</option>
          </select>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      txt_3: __( 'Accordion', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt_13: __( 'Spacing', process.env.VUE_APP_TEXTDOMAIN ),
      txt_14: __( 'Top Margin', process.env.VUE_APP_TEXTDOMAIN ),
      txt_15: __( 'Text', process.env.VUE_APP_TEXTDOMAIN ),
      txt_16: __( 'Icon', process.env.VUE_APP_TEXTDOMAIN ),
      txt_17: __( 'Choose Your Style', process.env.VUE_APP_TEXTDOMAIN ),
      txt_18: __( 'Closed Icon Color', process.env.VUE_APP_TEXTDOMAIN ),
      txt_19: __( 'Closed Icon', process.env.VUE_APP_TEXTDOMAIN ),
      txt_20: __( 'Open Icon Color', process.env.VUE_APP_TEXTDOMAIN ),
      txt_21: __( 'Open Icon', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt_25: __( 'Background Color', process.env.VUE_APP_TEXTDOMAIN ),
  ...
  mounted() {
    let self = this;
    lodash.forOwn( this.shared.block_templates.accordion, function( value, key ) {
  ...
  methods: {
    ...
    add_item() {
      ...
      let index = this.block.settings.items.push({
        icon: lastitem.icon,
        txt: 'Accordion ' + ( parseInt( this.block.settings.items.length ) + 1 ),
        details: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam commodo velit ex, non ultrices leo auctor at.',
      });
