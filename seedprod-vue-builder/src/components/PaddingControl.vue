<template>
        ...
        <div class="sp-form-group">
          <label>{{txt_1}}</label>
          <div v-if="block.settings.paddingSync === false" class="sp-flex sp-items-center">
            ...
            <span class="sp-flex sp-flex-col sp-items-center sp-text-11px">
              <input
                type="number" min="0"
                v-model="block.settings.paddingRight"
            ...
            <span class="sp-flex sp-flex-col sp-items-center sp-text-11px">
              <input
                type="number" min="0"
                v-model="block.settings.paddingLeft"
                class="sp-form-input focus:sp-border-neutral-20 sp-w-16 sp-rounded-tl-none sp-rounded-bl-none"
              />
              {{txt_5}}
            </span>
            <span class="sp-ml-2 sp-text-neutral-20" @click="sync_padding(true)">
              <svg
          ...
          <div v-else class="sp-flex sp-items-center">
            ...
            <span class="sp-ml-2 sp-text-primary" @click="sync_padding(false)">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
...
export default {
  ...
  data() {
    return {
      txt_1: __( 'Padding', process.env.VUE_APP_TEXTDOMAIN ),
  ...
  methods: {
    sync_padding( sync ) {
      this.block.settings.paddingSync = sync;

      //if (sync) {
      this.block.settings.paddingLeft = this.block.settings.paddingTop;
      this.block.settings.paddingBottom = this.block.settings.paddingTop;
      this.block.settings.paddingRight = this.block.settings.paddingTop;
