      ...
      >
        ...
        <div class="sp-form-group">
          ...
          <input
            :name="'Url'"
            type="text"
            v-model="block.settings.screenName"
        ...
        <div class="sp-form-group sp-flex sp-justify-between sp-items-center sp-mt-4">
          ...
          >
              <toggle-button
                  ...
                  v-model="block.settings.showScreenName"
                />&nbsp;
                <span
                  v-if="block.settings.showScreenName"
        ...
        <div class="sp-form-group sp-flex sp-justify-between sp-items-center sp-mt-4">
          ...
          >
                <toggle-button
                    ...
                    :sync="true"
                    style="margin-bottom: 0"
                    v-model="block.settings.showCount"
                  />&nbsp;
                  <span
                    v-if="block.settings.showCount"
                    class="sp-text-11px sp-uppercase sp-pl-1 sp-font-bold"
                  >{{ txt__2 }}</span>
                  <span
                    v-else
...
export default {
  name: 'TwitterFollowButtonOptions',
  mixins: [ helpers ],
  inject: {
    $validator: '$validator',
  },
  data() {
    return {
      ...
      txt_9: __( 'Twitter Screen Name', process.env.VUE_APP_TEXTDOMAIN ),
      txt_10: __( 'Show Screen Name', process.env.VUE_APP_TEXTDOMAIN ),
      txt_11: __( 'Show Count', process.env.VUE_APP_TEXTDOMAIN ),
  ...
  mounted() {
    ...
    setTimeout( function() {
      twttr.widgets.load(
            document.getElementById('sp-twitterfollowbutton-builder-' + self.block.id)
      );
    }, 600 );
    /*
    setTimeout( function() {
      self.onTwitterFollowButtonChange();
    }, 500 );
    */
  },
  created() {
    ...
    onTwitterFollowButtonChange() {
      ...
      twttr.ready(function (twttr) {
            ...
            setTimeout( function() {
                ...
                document.getElementById( 'sp-twitterfollowbutton-builder-' + self.block.id ).innerHTML = ' ';

                window.twttr.widgets.createFollowButton(
                  self.block.settings.screenName,
                  document.getElementById('sp-twitterfollowbutton-builder-' + self.block.id),
                  {
                    showScreenName: self.block.settings.showScreenName,
                    showCount: self.block.settings.showCount,
                    size: self.block.settings.buttonSize,
  ...
  watch: {
      'block.settings.showScreenName': function( newVal, oldVal ) {
        this.onTwitterFollowButtonChange();
      },
      'block.settings.buttonSize': function( newVal, oldVal ) {
        this.onTwitterFollowButtonChange();
      },
      'block.settings.showCount': function( newVal, oldVal ) {
        this.onTwitterFollowButtonChange();
      },
      'block.settings.screenName': function( newVal, oldVal ) {
        this.onTwitterFollowButtonChange();
      },
