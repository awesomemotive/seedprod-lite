  ...
  <div>
    ...
    <div v-if="view == 1">
      ...
      <div class="seedprod-sidebar-action-body" v-if="section_1">
        ...
        <div class="sp-form-group">
          <label>{{txt_21}}</label>
          <SliderControl v-model="block.settings.numComments" :min="5" :max="20" :step="1" />
        </div>

        <div class="sp-form-group">
          <label>{{txt_9}}</label>
          <select v-model="block.settings.orderBy" class="sp-form-select">
            <option value="social">{{txt_10}}</option>
            <option value="reverse_time">{{txt_11}}</option>
            <option value="time">{{txt_12}}</option>
          </select>
        </div>

        <div class="sp-form-group">
          <label>{{txt_17}}</label>
          <select v-model="block.settings.colorScheme" class="sp-form-select">
            <option value="light">{{txt_18}}</option>
            <option value="dark">{{txt_19}}</option>
          </select>
        </div>
        ...
        >
          ...
          >
            <toggle-button
              ...
              v-model="block.settings.lazyLoad"
            />&nbsp;
            <span
              v-if="block.settings.lazyLoad"
        ...
        <div class="sp-form-group">
          <label>{{txt__5}}</label>
          <select v-model="block.settings.targetUrl" class="sp-form-select">
            <option value="currentpage">{{txt__6}}</option>
            <option value="customlink">{{txt__7}}</option>
          </select>
        </div>

        <div class="sp-form-group" v-if="block.settings.targetUrl == 'customlink'">
              <div><label>{{txt__8}}</label></div>
              <input
                :name="'Link'"
                type="text"
                v-model="block.settings.customlinkurl"
                v-validate="'required'"
        ...
        <div class="sp-form-group" v-show="block.settings.targetUrl == 'currentpage'">
          <label>{{txt__9}}</label>
          <select v-model="block.settings.targetUrlFormat" class="sp-form-select">
            <option value="plainpermalink">{{txt__10}}</option>
            <option value="prettypermalink">{{txt__11}}</option>
          </select>
        </div>
...
export default {
  ...
  data() {
    return {
      txt__1: __( 'Lazy Loading', process.env.VUE_APP_TEXTDOMAIN ),
      txt__2: __( 'Yes', process.env.VUE_APP_TEXTDOMAIN ),
      txt__3: __( 'No', process.env.VUE_APP_TEXTDOMAIN ),
      txt__4: __( 'Faces', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt__8: __( 'Link', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt_9: __( 'Order By', process.env.VUE_APP_TEXTDOMAIN ),
      txt_10: __( 'Social', process.env.VUE_APP_TEXTDOMAIN ),
      txt_11: __( 'Reverse Time', process.env.VUE_APP_TEXTDOMAIN ),
      txt_12: __( 'Time', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt_21: __( 'Number of Comments', process.env.VUE_APP_TEXTDOMAIN ),
  ...
  methods: {
    onFBSettingChange() {
      FB.XFBML.parse( document.getElementById( '#sp-fbcomments-' + this.block.id ) );
    },
  },
  watch: {
    'block.settings.orderBy': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.lazyLoad': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.colorScheme': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.numComments': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.targetUrl': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
    'block.settings.customlinkurl': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },
  },
  components: {
