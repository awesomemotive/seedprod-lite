    ...
    <div v-if="view == 1">
      ...
      >
        ...
        >
          ...
          >
            <toggle-button
              ...
              v-model="block.settings.hideHeader"
            />&nbsp;
            <span
              v-if="block.settings.hideHeader"
        ...
        >
          ...
          >
            <toggle-button
              ...
              :sync="true"
              style="margin-bottom: 0"
              v-model="block.settings.hideFooter"
            />&nbsp;
            <span
              v-if="block.settings.hideFooter"
              class="sp-text-11px sp-uppercase sp-pl-1 sp-font-bold"
            >{{ txt__2 }}</span>
            <span
              v-else
        ...
        >
          ...
          >
            <toggle-button
              ...
              :sync="true"
              style="margin-bottom: 0"
              v-model="block.settings.hideBorders"
            />&nbsp;
            <span
              v-if="block.settings.hideBorders"
              class="sp-text-11px sp-uppercase sp-pl-1 sp-font-bold"
            >{{ txt__2 }}</span>
            <span
              v-else
        ...
        >
          ...
          >
            <toggle-button
              ...
              :sync="true"
              style="margin-bottom: 0"
              v-model="block.settings.transparent"
            />&nbsp;
            <span
              v-if="block.settings.transparent"
              class="sp-text-11px sp-uppercase sp-pl-1 sp-font-bold"
            >{{ txt__2 }}</span>
            <span
              v-else
        ...
        >
          ...
          >
            <toggle-button
              ...
              :sync="true"
              style="margin-bottom: 0"
              v-model="block.settings.hideScrollbar"
            />&nbsp;
            <span
              v-if="block.settings.hideScrollbar"
              class="sp-text-11px sp-uppercase sp-pl-1 sp-font-bold"
            >{{ txt__2 }}</span>
            <span
              v-else
        ...
        >
          ...
          >
            <toggle-button
              ...
              :sync="true"
              style="margin-bottom: 0"
              v-model="block.settings.showReplies"
            />&nbsp;
            <span
              v-if="block.settings.showReplies"
              class="sp-text-11px sp-uppercase sp-pl-1 sp-font-bold"
            >{{ txt__2 }}</span>
            <span
              v-else
        ...
        <div class="sp-form-group">
          ...
          >{{ errors.first("Twitter Handle") }}</span>
        </div>
        ...
        <div class="sp-form-group">
          ...
          <ColorPicker
            :label="txt_33"
            :color="block.settings.borderColors"
            v-model="block.settings.borderColors"
          />
        </div>
        -->
...
export default {
  name: 'TwitterEmbedTimelineOptions',
  mixins: [ helpers ],
  inject: {
    $validator: '$validator',
  },
  data() {
    return {
      txt_t1: __( 'Hide Header', process.env.VUE_APP_TEXTDOMAIN ),
      txt_t2: __( 'Hide Footer', process.env.VUE_APP_TEXTDOMAIN ),
      txt_t3: __( 'Hide Borders', process.env.VUE_APP_TEXTDOMAIN ),
      txt_t4: __( 'Transparent', process.env.VUE_APP_TEXTDOMAIN ),
      txt_t5: __( 'Hide Scrollbar', process.env.VUE_APP_TEXTDOMAIN ),
      txt__2: __( 'Yes', process.env.VUE_APP_TEXTDOMAIN ),
      txt__3: __( 'No', process.env.VUE_APP_TEXTDOMAIN ),
      txt__4: __( 'Show Replies', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt__8: __( 'Twitter Handle', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt_21_: __( 'Height', process.env.VUE_APP_TEXTDOMAIN ),
      ...
      txt_33: __( 'Border Color', process.env.VUE_APP_TEXTDOMAIN ),
  ...
  mounted() {
    ...
    setTimeout( function() {
      twttr.widgets.load(
            document.getElementById('sp-twitterembedtimeline-builder-' + self.block.id)
      );
    }, 600 );
    /*
    setTimeout( function() {    
        self.changeChromeData();
    }, 500 );
    */
  },
  created() {
  ...
  methods: {
    onTwitterEmbedTimelineChange() {

          let self = this;

          let timelineid = this.block.settings.tweeterid;
          ...
          twttr.ready(function (twttr) {
              ...
              setTimeout( function() {
                    document.getElementById( 'sp-twitterembedtimeline-builder-' + self.block.id ).innerHTML = ' ';
                    ...
                    window.twttr.widgets.createTimeline( {
                        ...
                        //cards: this.cards,
                        showReplies : self.block.settings.showReplies,
                        width: self.block.settings.width,
                        height: self.block.settings.height,
                        chrome: self.chrome,
                        align: self.block.settings.align,
                        //borderColor: self.block.settings.borderColors,
    ...
    changeChromeData(){
      ...
      let cardsdata = '';
      if ( true == this.block.settings.hideHeader ) {
        cardsdata += ' noheader';
      }
      if ( true == this.block.settings.hideFooter ) {
        cardsdata += ' nofooter';
      }
      if ( true == this.block.settings.hideBorders ) {
        cardsdata += ' noborders';
      }
      if ( true == this.block.settings.transparent ) {
        cardsdata += ' transparent';
      }
      if ( true == this.block.settings.hideScrollbar ) {
        cardsdata += ' noscrollbar';
      }

      this.chrome = cardsdata;
      this.onTwitterEmbedTimelineChange();
      
    }
  },
  watch: {
    
    'block.settings.hideHeader': function( newVal, oldVal ) {
      this.changeChromeData();
    },
    'block.settings.hideFooter': function( newVal, oldVal ) {
      this.changeChromeData();
    },
    'block.settings.hideBorders': function( newVal, oldVal ) {
      this.changeChromeData();
    },
    'block.settings.transparent': function( newVal, oldVal ) {
      this.changeChromeData();
    },
    'block.settings.hideScrollbar': function( newVal, oldVal ) {
      this.changeChromeData();
    },
    'block.settings.showReplies': function( newVal, oldVal ) {
      this.onTwitterEmbedTimelineChange();
    },
    ...
    'block.settings.height': function( newVal, oldVal ) {
      this.onTwitterEmbedTimelineChange();
    },
    'block.settings.borderColors': function( newVal, oldVal ) {
      this.onTwitterEmbedTimelineChange();
    },
