      ...
      <div class="seedprod-sidebar-action-body" v-if="section_1">
        <div class="sp-form-group">
          <div>
            ...
            <!--
            <label v-if="shared.settings.facebook_app_id==''">
                {{txt_33}}
                <router-link
            ...
            <label v-else>
                {{txt_35}}
                <strong>
                    {{shared.settings.facebook_app_id}}
                </strong>
                <router-link
                  tag="a" class="sp-text-primary"
                  :to="{
        ...
        <div class="sp-form-group">
          <label>{{txt_6}} </label>
          <select v-model="block.settings.buttonType" class="sp-form-select">
            <option value="like">{{txt_7}}</option>
            <option value="recommend">{{txt_8}}</option>
          </select>
        </div>
        -->
        ...
        >
          ...
          >
            <toggle-button
              ...
              v-model="block.settings.shareButton"
            />&nbsp;
            <span
              v-if="block.settings.shareButton"
        ...
        >
          ...
          >
            <toggle-button
              ...
              :sync="true"
              style="margin-bottom: 0"
              v-model="block.settings.showFaces"
            />&nbsp;
            <span
              v-if="block.settings.showFaces"
              class="sp-text-11px sp-uppercase sp-pl-1 sp-font-bold"
              >{{ txt__2 }}</span
            >
            <span
        ...
        <div class="sp-form-group">
          <label>{{txt_21}}</label>
          <SliderControl v-model="block.settings.width" :min="100" :max="1000" :step="50" />
        </div>
        -->
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      txt__1: __( 'Share Button', process.env.VUE_APP_TEXTDOMAIN ),
  ...
  methods: {
    onFBSettingChange() {
      FB.XFBML.parse( document.getElementById( '#sp-fblike-' + this.block.id ) );
    },
  },
  watch: {

    /*
      "block.settings.buttonType": function(newVal, oldVal) {
    ...
    'block.settings.shareButton': function( newVal, oldVal ) {
      this.onFBSettingChange();
    },

    /*
      "block.settings.showFaces": function(newVal, oldVal) {
        this.onFBSettingChange();
      },*/
